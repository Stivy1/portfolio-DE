...
def get_last_batch(logger: object):
  #USAR PARA DESPLIEGUES
    ...
    query = """SELECT 
                ...
                SUBSTR(CONCAT("T.V ", o.vendor_phone, " N.V ", UPPER(o.vendor_name)),1,54) AS F_430_NOTAS, 
            ...
            FROM (
                SELECT
                      ...
                      TRANSLATE(IFNULL(CAST(v.name AS STRING), ""), 'ÁÉÍÓÚÑ', 'AEIOUN' AS vendor_name,
                      CASE WHEN o.applied_discount != 0 THEN CONCAT("DESCUENTO: ",applied_discount) ELSE "NO APLICA DESCUENTO" END AS discount,
                      sucursal,
                      CASE WHEN supplier_id = 'd4be9648-9a0a-4946-aff4-d0a8a3a559a3' then "1998" else "2000" end as codigo_vendedor,
                      CASE WHEN supplier_id = 'd4be9648-9a0a-4946-aff4-d0a8a3a559a3' then "160" else "150" end as cia,
                      CASE WHEN supplier_id = 'd4be9648-9a0a-4946-aff4-d0a8a3a559a3' then "160" else "150" end as fact,
                      CASE WHEN supplier_id = 'd4be9648-9a0a-4946-aff4-d0a8a3a559a3' then "381" else "311" end as bodega,
                      CASE WHEN supplier_id = 'd4be9648-9a0a-4946-aff4-d0a8a3a559a3' then "160" else "150" end as movto,
                  ...
                  INNER JOIN (
                    SELECT 
                      ...
                      a.document_clean,
                      a.sucursal 
                    FROM `treintaco-sandbox.b2b_tropi.tropi_documents_clean_pereira` a
                  ...
                  WHERE supplier_id in ('d4be9648-9a0a-4946-aff4-d0a8a3a559a3', 'ef5f50d9-673b-4a49-b3d7-08ad6c5429c3')
                  AND o.id NOT IN (SELECT order_id FROM `treintaco-sandbox.b2b_tropi.orders_uploaded_pereira`)
