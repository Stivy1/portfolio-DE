...
import pandas as pd
logger = get_logger('Tropi Pereira Orders', "DEBUG")
...
text = '{} - PEREIRA.txt'.format(current_date)
...
if caracteres >= 38:
    ...
    response_BQ = dataframe_to_BQ(df, "treintaco-sandbox.b2b_tropi.orders_uploaded_pereira", logger)
    logger.debug(F"{response_BQ}")
    response_drive = text_to_drive(text, "1dMt_1yrhkXR-SebnFrv5ekgd1qj5ljWP")
