...
def get_last_batch(logger: object):
  #USAR PARA DESPLIEGUES
    ...
    #Logina nueva
    post_day = "CURRENT_DATE('-5:00') + 1 AND CURRENT_DATE('-5:00') + 2" # Por defecto Lunes a jueves
    if week_day == 5 and is_holiday_date(next_day(2)) is True: #Sabado con Lunes festivo
        post_day = "CURRENT_DATE('-5:00') + 3 AND CURRENT_DATE('-5:00') + 4"
    elif week_day == 5 and is_holiday_date(next_day(2)) is False: #Sabado con lunes normal
        post_day = "CURRENT_DATE('-5:00') + 2 AND CURRENT_DATE('-5:00') + 3"
    elif week_day == 4 and is_holiday_date(next_day(1)) is False: #Viernes
        post_day = "CURRENT_DATE('-5:00') + 1 AND CURRENT_DATE('-5:00') + 3" 
    elif week_day == 4 and is_holiday_date(next_day(3)) is True: #Viernes con lunes festivo
        post_day = "CURRENT_DATE('-5:00') + 1 AND CURRENT_DATE('-5:00') + 4" 
    else:
        post_day = "CURRENT_DATE('-5:00') + 1 AND CURRENT_DATE('-5:00') + 2" # Por defecto Lunes a jueves
    ...
    query = """SELECT 
            ...
            FROM (
                SELECT
                      ...
                      CASE WHEN supplier_id = '4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44' THEN 740 else 750 END as sucursal,
                      CASE WHEN supplier_id = '4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44' then "7502" else "7500" end as codigo_vendedor,
                      CASE WHEN supplier_id = '4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44' then "100" else "070" end as cia,
                      CASE WHEN supplier_id = '4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44' then "100" else "070" end as fact,
                      CASE WHEN supplier_id = '4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44' then "001" else "331" end as bodega,
                      CASE WHEN supplier_id = '4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44' then "100" else "070" end as movto,
                  ...
                  INNER JOIN (
                    SELECT 
                      ...
                      a.document_clean,
                      a.sucursal 
                    FROM `treintaco-sandbox.b2b_tropi.tropi_documents_clean_cali` a
                  ...
                  WHERE supplier_id in ('4dbb35d0-b82c-4bcc-9d99-bd21bb68ad44', '4c9191e8-2a10-4845-b000-2046df6de81d')
                  AND o.id NOT IN (SELECT order_id FROM `treintaco-sandbox.b2b_tropi.orders_uploaded_cali`)
                  AND COALESCE(effective_delivery_date, delivery_date) BETWEEN {0}
