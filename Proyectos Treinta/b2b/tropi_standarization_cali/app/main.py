...
import pandas as pd
logger = get_logger('Tropi Cali Orders', "DEBUG")
...
text = '{} - CALI.txt'.format(current_date)
...
if caracteres >= 38:
    ...
    response_BQ = dataframe_to_BQ(df, "treintaco-sandbox.b2b_tropi.orders_uploaded_cali", logger)
    logger.debug(F"{response_BQ}")
    response_drive = text_to_drive(text, "1Vqvz-tCnElPdaHpyXH53Lctu3VdH-e-b")
