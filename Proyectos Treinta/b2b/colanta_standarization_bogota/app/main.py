...
for i, order in df.iterrows():
    ...
    tropi_products = [
        TropiProduct(
            ...
            F_431_REFERENCIA_ITEM=product['F_431_ID_ITEM'],
            F_431_FECHA_ENTREGA=order['F_430_FECHA_ENTREGA'],
            F_431_NUM_DIAS_ENTREGA=order['F_430_NUM_DIAS_ENTREGA'],
        ...
        for idx, product in zip(
            ...
            if product['F_431_ID_UNIDAD_MEDIDA'] != "0047219"
        ]
    ...
    tropi_order = TropiOrder(
        ...
        F_430_NUM_DIAS_ENTREGA=order['F_430_NUM_DIAS_ENTREGA'],
...
if caracteres >= 38:
    ...
    response_BQ = dataframe_to_BQ(df, "data-production-337318.b2b_colanta.orders_uploaded_bogota", logger)
    logger.debug(F"{response_BQ}")
    response_drive = text_to_drive(text, "1s7RmXEA2eWBzG5VlRnvyJ3_0Na6by_E7")
