...
def get_last_batch(logger: object):
  #USAR PARA DESPLIEGUES
    ...
    query = """SELECT 
                ...
                o.external_id AS F_430_NUM_DOCTO_REFERENCIA,
                ...
                ABS(DATE_DIFF(DATE(delivery_date), DATE(order_date), DAY)) AS F_430_NUM_DIAS_ENTREGA,
                od.quantity AS F_431_CANT_PEDIDA_BASE,
                pr.sku AS F_431_ID_ITEM,
                COALESCE(p.external_id,'UND') AS F_431_ID_UNIDAD_MEDIDA,
                ...
                SELECT
                  ...
                  INNER JOIN (
                    SELECT 
                      b.COD_TERCERO as documento,
                      a.document_clean,
                      a.sucursal 
                    FROM `data-production-337318.b2b_colanta.colanta_documents_clean_bogota` a
                    INNER JOIN `data-production-337318.b2b_colanta.clients_uploaded` b
                    ON a.document_clean = CAST(b.COD_TERCERO AS STRING)
                  ) tdoc
                    ON tdoc.documento = TRIM(si.document)
                  INNER JOIN `treintaco-lz.treintagp_b2b.orders_group` og ON o.order_group_id = og.id
                  WHERE supplier_id in ('03fbb17e-53f3-43ce-8a4f-59656f4ca2d5')
                  AND o.id NOT IN (SELECT order_id FROM `data-production-337318.b2b_colanta.orders_uploaded_bogota`)
    ...
    if df.empty is True:
        ...
        df_agg = df.groupby('ORDER_ID').agg(
                            ...
                            F_430_NUM_DIAS_ENTREGA =('F_430_NUM_DIAS_ENTREGA', 'min'),
