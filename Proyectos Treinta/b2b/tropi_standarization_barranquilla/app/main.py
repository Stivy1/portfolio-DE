...
import pandas as pd
logger = get_logger('Tropi Barranquilla Orders', "DEBUG")
...
text = '{} - BARRANQUILLA.txt'.format(current_date)
...
if caracteres >= 38:
    ...
    response_BQ = dataframe_to_BQ(df, "treintaco-sandbox.b2b_tropi.orders_uploaded_barranquilla", logger)
    logger.debug(F"{response_BQ}")
    response_drive = text_to_drive(text, "19ip61P05XF26LGM_lNzjMdMwuu6EAusf")
