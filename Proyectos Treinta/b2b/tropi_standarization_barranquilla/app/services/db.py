...
def get_last_batch(logger: object):
  #USAR PARA DESPLIEGUES
    ...
    query = """SELECT 
                ...
                SUBSTR(CONCAT("T.V ", o.vendor_phone, " BA ", UPPER(o.neighborhood)," D.A ",SUBSTR(UPPER(o.additional_information),1, 20)),1,54) AS F_430_NOTAS,
            ...
            FROM (
                SELECT
                      ...
                      CASE WHEN supplier_id = 'f3098a02-9a38-4a71-a1c2-fc29201d026c' THEN 580 else 580 END as sucursal,
                      CASE WHEN supplier_id = 'f3098a02-9a38-4a71-a1c2-fc29201d026c' then "1704" else "1705" end as codigo_vendedor,
                      CASE WHEN supplier_id = 'f3098a02-9a38-4a71-a1c2-fc29201d026c' then "019" else "019" end as cia,
                      CASE WHEN supplier_id = 'f3098a02-9a38-4a71-a1c2-fc29201d026c' then "019" else "019" end as fact,
                      CASE WHEN supplier_id = 'f3098a02-9a38-4a71-a1c2-fc29201d026c' then "951" else "951" end as bodega,
                      CASE WHEN supplier_id = 'f3098a02-9a38-4a71-a1c2-fc29201d026c' then "019" else "019" end as movto,
                  ...
                  INNER JOIN (
                    SELECT 
                      ...
                      a.document_clean,
                      a.sucursal 
                    FROM `treintaco-sandbox.b2b_tropi.tropi_documents_clean_barranquilla` a
                  ...
                  WHERE supplier_id in ('f3098a02-9a38-4a71-a1c2-fc29201d026c')
                  AND o.id NOT IN (SELECT order_id FROM `treintaco-sandbox.b2b_tropi.orders_uploaded_barranquilla`)
